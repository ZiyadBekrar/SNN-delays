"""Sweep over s_init for the single-shared-delay variant.

Produces: one delay per layer, against one per neuron or per connection.
Outputs:  trained_models/HAR/std_init_sweep/SNN_common_recurrent_delays/
Usage:    python experiments/sweeps/sweep_HAR_commondelay.py
...
# delay_std_init sweep counterpart of sweep_HAR_delaystd.py, but for a SINGLE model:
# SNN_common_recurrent_delays, which learns ONE shared recurrent delay per layer
# (common_recdel, recurrent_delays shape (1,)) instead of per-neuron axonal delays.
# For every (delay_std_init, seed) pair it trains the exact HAR pipeline and persists,
# per run:
...
# Runs land under exp/HAR/std_init_sweep/SNN_common_recurrent_delays/std<X>/seed<N>/,
# beside the four per-neuron/per-synapse families, for direct comparison. A distinct
# manifest_commondelay.json at the sweep root indexes these runs.
#
# common_recdel's scalar delay is broadcast to all neurons only on the v2 forward
# path (the triton/eventdriven kernels index the delay per-neuron). V2 is
# numerically identical to triton_exact, so results stay comparable to the existing
# sweep. FORWARD_VERSION is therefore 'v2' here.
#
# ``run_single`` trains ONE (delay_std_init, seed) run and returns its manifest row.
# It is reused by the sequential loop below and by
# parallel_sweep_HAR_commondelay.py (one process per run).
FORWARD_VERSION = os.environ.get("HAR_FORWARD_VERSION", "v2")  # broadcast-safe for the shared scalar delay


def _recdel_layers(model):
    """Return the axonal_recdel layers in model.layers order (covers common_recdel)."""
    return [m for m in model.layers if isinstance(m, axonal_recdel)]


def run_single(model_name, dsi, seed, sweep_root, device, *,
               round_pos=False, epochs_override="", batch_override="", subdir=None):
    ...
    std tag ``std<dsi>``. Faithful to sweep_HAR_delaystd.py (no test-time sigma/round
    beyond config). Returns the manifest row dict."""
    ...
    # Keep the shared-delay layers on the broadcast-safe v2 path.
    n_ax = 0
    ...
    print(f"forward_version: '{FORWARD_VERSION}' on {n_ax} recurrent layers")
...
if __name__ == "__main__":
    ...
    MODELS = [s for s in os.environ.get(
        "HAR_MODELS", "SNN_common_recurrent_delays").split(",") if s]
    ...
    for model_name in MODELS:
        for dsi in DELAY_STD_INITS:
            for run_seed in seed_list:
                ...
                with open(os.path.join(SWEEP_ROOT, 'manifest_commondelay.json'), 'w') as fid:
    ...
    for (model_name, dsi), accs in sorted(group_acc.items()):
        accs = np.array(accs)
        print(f"{model_name:30s} std={dsi:<3d} "
              f"test acc = {accs.mean():.4f} +/- {accs.std():.4f}  (seeds={list(seed_list)})")
    print(f"\nManifest: {os.path.join(SWEEP_ROOT, 'manifest_commondelay.json')}")
