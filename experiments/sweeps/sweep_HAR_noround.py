"""Sweep over s_init, with the delays left fractional.

Produces: the cost of projecting delays onto the integer grid.
Outputs:  trained_models/HAR/std_init_sweep_noround/
Usage:    python experiments/sweeps/sweep_HAR_noround.py
...
# test, and train() never rounds/clamps. The forward mask at sigma=0 (HAR default)
# honors the fractional delay via 2-tap linear interpolation. The rounded runs
# already live under exp/HAR/std_init_sweep/. These unrounded runs land in the
# sibling exp/HAR/std_init_sweep_noround/ so analyze_rounding.py can overlay them.
...
# A manifest_noround.json at the sweep root indexes all runs.
#
...
# parallel_sweep_HAR_noround.py (one process per run).
...
def run_single(model_name, dsi, seed, sweep_root, device, *,
    ...
    std tag ``std<dsi>``. Faithful to sweep_HAR_delaystd.py, but ``round_pos``
    defaults to False (the whole point of this sweep). Returns the manifest row."""
...
if __name__ == "__main__":
    ...
    SWEEP_ROOT = os.environ.get("HAR_SWEEP_ROOT", './exp/HAR/std_init_sweep_noround')
    ...
    for model_name in MODELS:
        for dsi in DELAY_STD_INITS:
            for run_seed in seed_list:
                ...
                with open(os.path.join(SWEEP_ROOT, 'manifest_noround.json'), 'w') as fid:
    ...
    print(f"\nManifest: {os.path.join(SWEEP_ROOT, 'manifest_noround.json')}")
