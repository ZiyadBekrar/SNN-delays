"""Run the noround sweep, one process per run across the GPUs.

Sweeps the initialization width, with delays left fractional.
...
Usage:    python experiments/sweeps/parallel_sweep_HAR_noround.py
"""
...
# Repo-root on sys.path so the worker's `from sweep_HAR_noround import ...`
# (and that module's `configs`/`delrec` imports) resolve when run as
# `python experiments/sweeps/parallel_sweep_HAR_noround.py`.
...
def worker_main(args):
    ...
    import torch
    from sweep_HAR_noround import run_single
...
def orchestrate(args):
    ...
    if interrupted:
        print(f"[interrupt] workers terminated. Partial manifest: "
              f"{os.path.join(sweep_root, 'manifest_noround.json')}", flush=True)
...
def _summarize(results, params):
    ...
    print(f"\nManifest: {os.path.join(params['sweep_root'], 'manifest_noround.json')}")
