"""Run the commondelay sweep, one process per run across the GPUs.

Sweeps the initialization width, for the shared-delay variant.
...
Usage:    python experiments/sweeps/parallel_sweep_HAR_commondelay.py
"""
...
# Repo-root on sys.path so the worker's `from sweep_HAR_commondelay import ...`
# (and that module's `configs`/`delrec` imports) resolve when run as
# `python experiments/sweeps/parallel_sweep_HAR_commondelay.py`.
...
#       (Env HAR_MAX_RUNS_PER_GPU overrides.) The shared-delay model is memory-light,
#       so 2 is comfortable.
MAX_RUNS_PER_GPU = 2
...
def build_triples():
    """(model, std, seed) list + sweep params."""
...
def worker_main(args):
    ...
    import torch
    from sweep_HAR_commondelay import run_single
...
def orchestrate(args):
    ...
    if interrupted:
        print(f"[interrupt] workers terminated. Partial manifest: "
              f"{os.path.join(sweep_root, 'manifest_commondelay.json')}", flush=True)
...
def _summarize(results, params):
    ...
    print(f"\nManifest: {os.path.join(params['sweep_root'], 'manifest_commondelay.json')}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--round-pos", default="0")
