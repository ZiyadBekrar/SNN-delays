"""Training and evaluation loop for AL.
...
def train(train_loader, model, optimizer, epoch, device, config, penalize_spikes=False, logger=no_log):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        loss = calc_loss_AL(outputs, targets)
        ...
        correct += calc_metric_AL(outputs, targets)
        total += targets.size(0)

        for opt in optimizer: opt.zero_grad()
        loss.backward()
        # neuroseqbench AL clips the global gradient norm (clip_grad_norm_) at grad_clip=1.0.
