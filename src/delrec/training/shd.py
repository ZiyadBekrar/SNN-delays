"""Training and evaluation loop for SHD.
...
from delrec.networks_shd import dcls_module, modified_batchnorm, spike_registrator, SNN_vanilla_recurrent
...
def test(test_loader, model, epoch, device, config, penalize_spikes=False, input_transform=None, logger=no_log):
    test_loss = 0
    accuracy = 0

    model.eval()
    
    if getattr(config, 'round_pos_each_epoch', True) and hasattr(model, 'round_pos'):
        model.round_pos()
