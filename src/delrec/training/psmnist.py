"""Training and evaluation loop for PS-MNIST.
...
def train(train_loader, model, optimizer, epoch, device, config, perm, penalize_spikes=False, logger=no_log):
    train_loss = 0
    correct    = 0
    total      = 0
...
def test(test_loader, model, epoch, device, config, perm, penalize_spikes=False, logger=no_log):
    test_loss = 0
    correct = 0
    total = 0
